...
- **Qwen3.8-27B + shoplifting LoRA** (yes/no scorer): its runbook `model/.plans/qwen-lora-zrt.md` was removed in commit d79bacd (git history); not in the current architecture. zrt's proxy **only routes its service label, not LoRA names**. Expose vLLM's socket with the localhost bridge in that runbook (`socat` on 127.0.0.1:8081); re-create it after reboots.
...
## Web app
- `cd webapp && npm run dev` → http://localhost:3000 (binds all interfaces). **Running as of 2026-09-26** (detached dev server). Lint and typecheck: `npm run lint && npm run typecheck`.
...
- Plan: `webapp/.plans/hermes-watch-agent.md` (local plan, not in the repo). [Hermes Agent](https://hermes-agent.nousresearch.com) in Docker container `kryptonx-hermes` (state in `~/.hermes-kryptonx`, 2 GB RAM and 1 CPU cap, no GPU, no published ports). Runs a 5-minute update and an hourly digest; the briefing shows on `/overview`.
...
## Active work: `model/.plans/local-vlm-quality.md` (local plan, not in the repo)
...
- Removed in commit d79bacd (see git history): GLM-4.5V arm (**skipped**), Nemotron fine-tune and serving plans (superseded by Qwen3-VL), Qwen3.8 arm and bake-off plans, the LoRA scorer runbook, and the finished web app plans (HawkWatch parity, model integration, roadmap, YOLO boxes, dashboard refresh).

## Conventions
- **Plans:** discuss and plan first on big work. Save approved plans as markdown (Context, ordered steps, Verification, status line): web app → `webapp/.plans/`, model → `model/<Model>/plans/` (cross-model → `model/.plans/`). Keep status lines current. `.plans/` folders are git-ignored since 2026-09-26: plans stay on this machine and never go to GitHub.
