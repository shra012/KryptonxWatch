...
2. Pull NVIDIA's ARM64 CUDA 13 containers for this GPU class: an NGC PyTorch image for training and an NGC or vLLM image for serving. Record the image digests in `model/GLM-4.5-VL/env/glm/images.lock`.
3. Inside the training container, install and pin transformers ≥ 4.57.1, peft, accelerate, bitsandbytes, LLaMA-Factory, huggingface_hub and PyAV/decord. Record them in `model/GLM-4.5-VL/env/glm/requirements.lock`.

**Gate:** `torch.cuda.is_available()` is true in the container and a 4-bit matmul test (bitsandbytes) runs on the GPU.

**Result (2026-09-24): passed.** Base image `nvcr.io/nvidia/vllm:26.03.post1-py3` (already on the host; torch 2.11 with CUDA 13.2, transformers 4.57.5, vLLM 0.17.1, sees the GB10 as sm_121) plus peft 0.21.0, bitsandbytes 0.50.2 and accelerate 1.15.0 → `kryptonx/glm:dev`, built from `model/GLM-4.5-VL/env/glm/Dockerfile`. The NF4 matmul runs on the GPU. Pins are in `images.lock` and `requirements.lock`. No NGC PyTorch image was needed.
...
3. Write SHA-256 checksums of every safetensors shard to `model/GLM-4.5-VL/env/glm/weights.sha256`, and the revision to `model/GLM-4.5-VL/env/glm/model.json`. Weights are never committed; the root `.gitignore` already excludes model weights.

**Gate:** the checksums are recorded and the files load (`AutoConfig` plus the processor load without errors).

**Result (2026-09-24): passed.** 56 files, 201 GB on disk. All 47 large files match the Hub's SHA-256 (`model/GLM-4.5-VL/env/glm/weights.sha256`). Config (`glm4v_moe`) and `Glm4vProcessor` load in `kryptonx/glm:dev`. An 8 s window (16 frames at 320×240) becomes 873 input tokens, and thinking is disabled through the template's `/nothink` plus an empty think block.
...
**Result (2026-09-24): passed** (`model/GLM-4.5-VL/runs/glm45v/g2-pilot-box/`).
- Loading and quantising to NF4 takes about 13 min. Peak GPU memory is **58.4 GiB**, leaving about 50 GiB free.
- **2.2 s per UCF window** (320×240) and about 7.7 s per MERL window (920×680), median over 20 non-test windows.
- Scores are **deterministic**: all 20 repeats match exactly.
- **Scoring fix:** GLM-4.5V opens every answer with `<|begin_of_box|>`, so reading the first token's yes/no probability is invalid (first run: `g2-pilot/`). The scorer appends `<|begin_of_box|>` to the assistant turn and reads the next token. Yes/no tokens then carry at least 0.9999 of the probability, and the argmax is always yes or no. Every later experiment uses this answer prefix.
- These pilot scores come from random training/validation windows and say nothing about accuracy.
...
4. Outputs go to `model/GLM-4.5-VL/runs/glm45v/<exp>/<seed>/` (configuration, adapter, raw scores, `env.json` with the container digest, revision, precision and versions).
