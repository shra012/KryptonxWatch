...
test : window-level metrics on the locked test set for one or two score files. With two files
  ...
  python model/src/evaluate.py val --scores /data/runs/qwen38/exp3/.../val_scores.csv
  python model/src/evaluate.py test --scores ft.csv --baseline zeroshot.csv --out report.json
"""
...
STUDY = Path(__file__).resolve().parent.parent / "study/v1"
EPS = 1e-6
