...
export interface ModelStatus { configured: boolean; model?: string; chatModel?: string; provider?: string; options?: string[] }
...
function withSelection(status: ModelStatus): ModelStatus {
  ...
  return status.configured && picked && status.options?.includes(picked) ? { ...status, model: picked, chatModel: picked } : status;
}
...
export async function analyzeRecording(video: VideoRecord, src: string, onProgress: (p: AnalysisProgress) => void, signal: AbortSignal): Promise<AnalysisOutput> {
  ...
  const windows = planWindows(Number.isFinite(el.duration) && el.duration > 0 ? el.duration : video.duration);
  ...
  for (const w of windows) {
    ...
    const frames: Frame[] = [];
    for (const t of w.times) { await seek(el, t); frames.push({ seconds: t, image: grabFrame(el) }); }
