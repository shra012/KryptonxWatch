...
export function AppProvider({children}:{children:React.ReactNode}){
 const [videos,setVideos]=useState<VideoRecord[]>(sampleVideos); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [theme,setThemeState]=useState<"light"|"dark">("light"); const [alerts,setAlertsState]=useState(true); const [smsAlerts,setSmsAlertsState]=useState(false); const [toast,setToast]=useState("");
 useEffect(()=>{setThemeState(localStorage.getItem("kryptonxwatch-theme")==="dark"?"dark":"light");setAlertsState(localStorage.getItem("kryptonxwatch-alerts")!=="false");setSmsAlertsState(localStorage.getItem("kryptonxwatch-sms")==="true");videoRepository.list().then(stored=>{const hidden=hiddenSamples();setVideos([...sampleVideos.filter(v=>!hidden.includes(v.id)).map(v=>stored.find(s=>s.id===v.id)??v),...stored.filter(v=>v.source==="upload")]);}).catch(e=>setError(storageError(e))).finally(()=>setLoading(false));},[]);
 ...
 useEffect(()=>{localStorage.setItem("kryptonxwatch-sms",String(smsAlerts));},[smsAlerts]);
