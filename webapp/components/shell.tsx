...
import { BarChart3, Clapperboard, CloudUpload, LayoutDashboard, ListFilter, Moon, Settings2, ShieldCheck, Sun, X } from "lucide-react";
import { useApp } from "./app-provider";
const nav=[{href:"/",label:"Overview",icon:LayoutDashboard},{href:"/upload",label:"Upload video",icon:CloudUpload},{href:"/videos",label:"Video library",icon:Clapperboard},{href:"/detections",label:"Detection log",icon:ListFilter},{href:"/analytics",label:"Analytics",icon:BarChart3},{href:"/settings",label:"Preferences",icon:Settings2}];
...
return <div className="min-h-screen lg:flex">
 ...
 <div className="min-w-0 flex-1">
  <header className="sticky top-0 z-30 bg-base-100/90 backdrop-blur border-b border-base-300 px-4 lg:px-8 h-18 flex items-center justify-between gap-3">
   ...
   <div className="flex items-center gap-2"><span className="badge badge-info badge-soft hidden sm:inline-flex">Demo mode</span><button className="btn btn-ghost btn-circle" aria-label={theme==="light"?"Switch to dark mode":"Switch to light mode"} onClick={()=>setTheme(theme==="light"?"dark":"light")}>{theme==="light"?<Moon size={20}/>:<Sun size={20}/>}</button><span className="avatar avatar-placeholder"><span className="bg-primary/10 text-primary rounded-full w-9 text-xs font-bold">KW</span></span></div>
  </header>
