...
import { boxAt, mergeOverlapping } from "@/lib/vlm/boxes";
import { isSecurityDetection, timecode, type Detection, type VideoRecord } from "@/lib/types";

/** Seconds of footage around a detection that the tile loops: a little lead-in and the flagged span, at least 4 s. */
function loopSegment(d: Detection, duration: number) {
 // The flagged span only: keyframes now follow the suspect through the whole video, which would loop everything.
 const first = d.seconds, last = d.endSeconds ?? d.seconds;
...
/** `caption="category"` names the tile by its crime only (the landing page); the default shows the recording name. */
export function MonitorTile({video,detection,index,caption="title"}:{video:VideoRecord;detection:Detection;index:number;caption?:"title"|"category"}){
 ...
 const boxes=mergeOverlapping(video.detections.filter(isSecurityDetection).flatMap(d=>{const b=boxAt(d,t);return b?[{id:d.id,box:{...b,label:d.category}}]:[]}));
 return <Link href={`/videos/${video.id}?t=${Math.floor(detection.seconds)}`} className="group block relative overflow-hidden rounded-lg border border-base-300 bg-black hover:border-base-content transition-colors focus-visible:outline-2 focus-visible:outline-primary">
  <div className="relative aspect-video bg-black">
   ...
   {boxes.map(({id,box})=><div key={id} className="absolute border-[1.5px] border-white rounded-md shadow-[0_0_0_1px_rgba(0,0,0,.65)]" style={{left:`${box.x*100}%`,top:`${box.y*100}%`,width:`${box.width*100}%`,height:`${box.height*100}%`}}>
   ...
   <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent pt-8 pb-2 px-2.5">
    <div className="flex items-end justify-between gap-2">
     <span className="min-w-0"><span className="block text-xs font-medium text-white truncate">{caption==="category"?detection.category:video.title}</span>
      <span className="block font-mono text-[.6rem] text-white/60 truncate">{caption==="category"?"":`${detection.category} · `}{timecode(start)}–{timecode(end)}</span></span>
